...
void handle_new_connection(int fd, void *arg);
void handle_client_message(int fd, void *arg);
...
int main(void)
{
    ...
    reactor->maxfd = listener; // so far, it's this one
            ...
            { // we got one!!
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        for (j = 0; j <= reactor->maxfd; j++)
                        {
                            // send to everyone!
                            if (FD_ISSET(j, &reactor->fds))
                            {
...
void handle_new_connection(int fd, void *arg)
{
    // Convert arg to reactor pointer
